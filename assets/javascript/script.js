...
async function searchForTranslate() {
  ...
  const responseTranslate = await request.sendRequestAsyncWithRefresh(
    "GET",
    translateURL
  );
...
async function searchForContext() {
  ...
  let i = 0;
  document
    .querySelectorAll(".synonym__button")
    .forEach((s) => (s.textContent = responseContext[i++].Word));
