...
async function createCard() {
  synonymButtons.forEach((button) =>
    button.addEventListener("click", async function (event) {
      const ContextURL = API_URL.getDatamuseUrl(button.textContent);
      const responseContext = await request.sendRequestAsyncWithRefresh(
        "GET",
        ContextURL
      );
      ...
      document
        .querySelector(".wrap__context")
        .classList.remove("wrap__context-flex");
...
backToSynonymsBtn.addEventListener("click", function (event) {
  ...
  introductionContext.children[0].textContent =
    "Choose any synonym which somehow bound with your word.";
});
...
async function rememberNewCard() {
  const authModel = JSON.parse(localStorage.getItem("accessToken"));
  const userId = authModel.Id;
  const data = await request.sendRequestAsyncWithRefresh(
    "GET",
    `https://learn-application.herokuapp.com/api/card/vocabulary?userId=${userId}`
  );

  const wordAlreadyExists = containsWord(data, rememberCard.word);
  if (!wordAlreadyExists) {
    const result = await request.sendRequestAsync(
      REMEMBER_CARD_URL,
      rememberCard
    );
    ...
    if (result.ok === true) {
      ...
      document
        .querySelector(".wrap__context")
        .classList.add("wrap__context-flex");
    }
  } else {
    introductionContext.children[0].textContent =
      "Current word already exists in your dictionary.";
  }
}
